...
a1=histogram(finalTestErrors)
title('Distribution of the outer folds test-error values')
...
KSTest_Reject = kstest(stdFinalTestErrors)
...
% distribution whose median (and mean, if it exists) is zero, does NOT
...
finalTestErrors = [0.2731    0.2467    0.2808    0.3150] % can be commented if you possess 'finalResultsGS5.mat'
...
% ANS: The mean test error is of 0.2443 and is sufficiently away from the
% 0,5 random distribution, so that it might well be statistically
% different...
...
% ANS: The obtain p-value is of 5.6215e-04 = 0.05%, which means that if the
% null hypothesis Ho - "The samples come from a random distribution." - was
% true, then our distribution and any other distribution of the same size
% would have this 0.05% chance to exist or being even more divergent from
% the null hypothesis. Since we do have observed these results, it means
% that it is highly improbable that we got these 0.05% chance. So, it
% implies that the null hypothesis is to be rejected: our samples do not
% come from the sample distribution. (Simplier: p < 0.05 (5%), reject Ho).
% To confirm, h = 1 means "rejected" in this matlab function. 
...
% ANS: The histogram looks, indeed, normal.
...
% But first creates a vector of mean(finalTestErrors) of the same
% dimensions. If we don't we cannot apply
meanVect = [mean(finalTestErrors) mean(finalTestErrors) mean(finalTestErrors) mean(finalTestErrors)]
stdFinalTestErrors = ((finalTestErrors - meanVect)/std(finalTestErrors))
...
>>>>>>> efe6a65cb5142b414d8720da908bad3d2113ce01
